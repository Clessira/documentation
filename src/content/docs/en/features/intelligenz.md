---
title: Intelligence
description: Auto-confirm, undo toast, adaptive intervals, and Focus detection — how Clessira adapts to your rhythm.
---

Clessira tries to stay out of the way when you are deep in a task and to ask only when there is something to ask about. The helpers below run quietly in the background. All toggles live under **Settings → Intelligence**.

## Auto-confirm

If you confirm the same activity three times in a row, Clessira assumes you are in flow. The prompt popover is then **skipped** and the running block continues silently — no popover, no sound, no interruption.
...
The [tracking interval](/en/features/tracking-interval/) is not set in stone — with this toggle on, Clessira tunes it to your behaviour:
...
You get quiet during focus sessions and stay fine-grained during reactive work. The base value (the one Clessira returns to) is still set under **Tracking interval**.
...
If a **macOS Focus** is active on your Mac — *Do Not Disturb*, *Presentation*, *Sleep*, a custom Focus —, Clessira suppresses the automatic prompt entirely. Neither popover nor sound interrupts you.

- Tracking still runs in the background — the current activity is not stopped.
- When the Focus ends, Clessira asks again on the next regular interval.
